 ...
 * could issue itself a MobiStack-admin token. Here MobiStack can check a signature and cannot
 * produce one.
 */
@Slf4j
@Component
public class IdentityKeySource {
    ...
    private boolean isStale() {
        return fetchedAt.plus(config.getJwksCacheTtl()).isBefore(Instant.now());
    }

    private void refreshIfAllowed() {
        Instant now = Instant.now();
        Duration sinceAttempt = Duration.between(lastAttemptAt, now);
        if (sinceAttempt.compareTo(config.getJwksMinRefreshInterval()) < 0) {
            return;
        }
